...
class Ajax {
    ...
    public function filter_modules() {
        ...
        // دسته‌بندی
        // نکته مهم: برای جلوگیری از مشکل slug فارسی و ماژول‌های چنددسته‌ای،
        // مقدار تب‌ها با term_id ارسال می‌شود. با این حال slug/name هم پشتیبانی می‌شود.
        if ( ! empty( $_POST['category'] ) && $_POST['category'] !== 'all' ) {
            ...
            if ( is_numeric( $raw_category ) ) {
                ...
                if ( ! $term ) {
                    $term = get_term_by( 'name', $raw_category, 'markil_category' );
                }
        ...
        // برچسب: هم term_id و هم slug/name پشتیبانی می‌شود تا فیلتر برچسب فارسی درست کار کند.
        if ( ! empty( $_POST['tag'] ) ) {
            ...
            if ( is_numeric( $raw_tag ) ) {
                ...
                if ( ! $term ) {
                    $term = get_term_by( 'name', $raw_tag, 'markil_tag' );
                }
                if ( $term && ! is_wp_error( $term ) ) {
        ...
        switch ( $_POST['orderby'] ?? 'newest' ) {
            ...
            case 'price_high':
                ...
                break;
            case 'rating':
                $args['meta_key'] = '_markil_rating';
                $args['orderby']  = 'meta_value_num';
                $args['order']    = 'DESC';
                break;
            default: // popular / favorites
                $args['meta_key'] = '_markil_favorites_count';
                $args['orderby']  = 'meta_value_num';
                $args['order']    = 'DESC';
        }

        // فیلترهای متا: قیمت، امتیاز و رایگان/پولی
        ...
        if ( $query->have_posts() ) {
            while ( $query->have_posts() ) {
                $query->the_post();
                $post_id = get_the_ID();
                $modules[] = $this->format_module( $post_id );
            }
            wp_reset_postdata();
        }
    ...
    public function get_module_detail() {
        ...
        $module = $this->format_module( $post_id, true );
    ...
    public function submit_review() {
        ...
        if ( ! is_user_logged_in() ) {
            wp_send_json_error( __( 'برای ثبت نظر باید وارد شوید', 'markil-modules' ) );
        }
        ...
        if ( $rating < 1 || $rating > 5 ) {
            wp_send_json_error( __( 'امتیاز نامعتبر', 'markil-modules' ) );
        }
        ...
        wp_send_json_success( [
            'message' => __( 'نظر شما با موفقیت ثبت شد', 'markil-modules' ),
            'rating'  => round( $avg, 1 ),
            'count'   => count( $reviews ),
        ]);
    }
    ...
    public function toggle_wishlist() {
        check_ajax_referer( 'markil_modules_nonce', 'nonce' );
        if ( ! is_user_logged_in() ) {
            wp_send_json_error( __( 'برای افزودن به علاقه‌مندی‌ها باید وارد شوید', 'markil-modules' ) );
        }
