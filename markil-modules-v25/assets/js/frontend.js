...
(function($) {
    ...
    function buildDetailHtml(m, icons) {
        ...
        var btn1Href = m.wc_product_id ? '?add-to-cart=' + encodeURIComponent(m.wc_product_id) : (m.permalink || '#');
        var btn2Href = m.detail_page || m.permalink || '#';

        // Secondary button: open full-detail modal if action is full_detail_modal or no detail_page
        var btn2IsModal = (m.btn_secondary_action === 'full_detail_modal') || (!m.detail_page && m.btn_secondary_action !== 'wc_add' && m.btn_secondary_action !== 'wc_page' && m.btn_secondary_action !== 'custom_url');
